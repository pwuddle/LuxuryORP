...
const STAFF_MEMBERS = [
  { id: "987654321098765432", name: "Loek | Perseus Sales", role: "Perseus Senior Sales" },
  { id: "112233445566778899", name: "Franklin | Perseus Manager", role: "Perseus Manager" },
...
export default function EmployeePanel({
  ...
  const [bonusSalary, setBonusSalary] = useState<number>(15000); // global base 15.000€
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-8">
                    ...
                    <div key={v.id} className={`p-4 rounded-lg bg-black/15 border flex flex-col md:flex-row justify-between items-start md:items-center gap-4 hover:border-[#A87E43]/40 transition-all ${outOfStock ? 'border-rose-500/20 bg-rose-500/5 opacity-80' : 'border-white/5'}`}>
                      ...
                      <div className="flex flex-wrap items-center gap-2 w-full md:w-auto justify-between md:justify-end">
                        ...
                        <button 
                          type="button"
                          onClick={() => {
                            if (confirm(`Weet u zeker dat u de ${v.brand} ${v.name} definitief wilt verwijderen van de website?`)) {
          ...
          {activeTab === "financieel" && (
            <div className="space-y-6 relative overflow-hidden">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                <div className={`p-5 rounded-xl ${bgCard} border ${borderCard} shadow flex items-center justify-between`}>
                  <div>
                    ...
                    <strong className="text-xl font-sans font-black text-green-400">{formatPrice(sales.reduce((sum, s) => sum + s.pricePaid, 0))}</strong>
                  </div>
                ...
                <div className={`p-5 rounded-xl ${bgCard} border ${borderCard} shadow flex items-center justify-between`}>
                  <div>
                    ...
                    <strong className="text-xl font-sans font-black text-[#A87E43]">{formatPrice(sales.reduce((sum, s) => sum + s.pricePaid, 0) * 0.25)}</strong>
                  </div>
                ...
                <div className={`p-5 rounded-xl ${bgCard} border ${borderCard} shadow flex items-center justify-between`}>
                  <div>
                    ...
                    <strong className={`text-xl font-black ${textPrimary}`}>{sales.length} Verkopen</strong>
                  </div>
              ...
              <div className={`p-6 rounded-xl ${bgCard} border ${borderCard} space-y-4 shadow-md`}>
                ...
                {/* Adjustments Panel */}
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 p-4 bg-black/15 rounded-lg border border-white/5 text-xs">
                  <div className="space-y-2">
                    <div className="flex justify-between font-bold text-white">
                      <span>Provisie Tarief (Verkoop Commissie):</span>
                      <span className="text-[#A87E43]">{commissionRate}% van orderprijs</span>
                    </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between font-bold text-white">
                      <span>Vast Basis Salaris (Bon):</span>
                      <span className="text-[#A87E43]">{formatPrice(bonusSalary)}</span>
                    </div>
                    <input type="range" min="0" max="50000" step="1000" value={bonusSalary} onChange={(e) => setBonusSalary(Number(e.target.value))} className="w-full text-[#A87E43] rounded cursor-pointer accent-[#A87E43]" />
                  </div>
                </div>
                ...
                <div className="space-y-4 max-h-[400px] overflow-y-auto pr-1">
                  {salespersonList.map(salesperson => {
                    const staffSales = sales.filter(s => s.salesperson === salesperson);
                    const staffTotalSalesVolume = staffSales.reduce((sum, s) => sum + s.pricePaid, 0);
                    const staffCommission = (staffTotalSalesVolume * commissionRate) / 100;
                    const totalStaffPay = staffCommission + bonusSalary;
                    ...
                    return (
                      <div key={salesperson} className="p-4 rounded-lg bg-black/25 border border-white/5 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 text-xs">
                        <div className="space-y-1">
                          ...
                          <span className="text-[10px] text-gray-500">Totaal Volume: <strong className="text-[#A87E43] font-sans">{formatPrice(staffTotalSalesVolume)}</strong> ({staffSales.length} verkopen)</span>
                        </div>
                        <div className="grid grid-cols-3 gap-4 text-left md:text-right">
                          <div>
                            <span className="text-[10px] text-gray-500 block">Provisie ({commissionRate}%)</span>
                            <strong className="text-white font-sans">{formatPrice(staffCommission)}</strong>
                          </div>
                          <div>
                            <span className="text-[10px] text-gray-500 block">Salaris Basis</span>
                            <strong className="text-white font-sans">{formatPrice(bonusSalary)}</strong>
                          </div>
                          <div>
      ...
      <AnimatePresence>
        {selectedStaffPayslip && (() => {
          const staffSales = sales.filter(s => s.salesperson === selectedStaffPayslip);
          const totalVolume = staffSales.reduce((sum, s) => sum + s.pricePaid, 0);
          const calculatedComm = (totalVolume * commissionRate) / 100;
          const calculatedTotal = calculatedComm + bonusSalary;
              ...
              <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className={`w-full max-w-lg p-6 rounded-xl ${isDarkMode ? "bg-[#2f3136]" : "bg-white"} border ${borderCard} space-y-6 text-xs`}>
                ...
                <div className="space-y-3 bg-black/25 p-4 rounded-lg font-mono text-gray-300">
                  ...
                  <div className="border-t border-dashed border-white/10 pt-2 space-y-1">
                    ...
                    <p className="flex justify-between text-[#A87E43] font-bold"><span>Commissie ({commissionRate}%):</span> <span>{formatPrice(calculatedComm)}</span></p>
                    <p className="flex justify-between"><span>Vast Basis Loon:</span> <span>{formatPrice(bonusSalary)}</span></p>
                  </div>
