...
export default function ClientPanel({ isDarkMode, user, requests, sales, deletedCustomerIds, onStartOAuth }: ClientPanelProps) {
  ...
  // Dynamically compute invoices for the logged-in user
  const invoices = useMemo(() => {
    if (!user) return [];
    return INITIAL_INVOICES.filter(inv => {
      if (inv.customerDiscordId !== user.id) return false;
      // If it is 'Betaald', ensure there is still a corresponding sale in sales!
      if (inv.status === "Betaald") {
        return sales.some(s => s.buyerDiscordId === inv.customerDiscordId && s.vehicleName === inv.vehicleName);
      }
      return true;
    }).concat(
      // Also add newly created sales that aren't in INITIAL_INVOICES!
      sales
        .filter(s => s.buyerDiscordId === user.id)
        .filter(s => !INITIAL_INVOICES.some(inv => inv.customerDiscordId === s.buyerDiscordId && inv.vehicleName === s.vehicleName))
        .map((s, idx) => ({
          id: `dyn_i_${s.id}`,
          invoiceNumber: `INV-2026-${100 + idx}`,
          customerName: s.buyerName,
          customerDiscordId: s.buyerDiscordId,
          vehicleName: s.vehicleName,
          amount: s.pricePaid,
          date: s.date,
          status: s.status as any || "Betaald"
        }))
    );
  }, [user, sales]);
